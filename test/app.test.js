...
test('første start: opsætning kræves, sider omdirigerer til /login', async () => {
  ...
  assert.deepEqual(status.json, { setupRequired: true, setupTokenRequired: false, usesEnvPassword: false, authenticated: false });
...
test('opsætning: for kort adgangskode afvises, gyldig logger ind', async () => {
  ...
  const status = await request('GET', '/api/auth/status');
  assert.deepEqual(status.json, { setupRequired: false, setupTokenRequired: false, usesEnvPassword: false, authenticated: true });
